    ...
    try {
      ...
      if (action === "trust-cert") {
        ...
        const keyToUse = selectedApiKey?.trim()
          ...
          || (!cloudEnabled ? "sk_routerdone" : null);
        res = await fetch("/api/cli-tools/antigravity-mitm", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="px-2 py-2 rounded-lg bg-surface/50 border border-border/50 flex flex-col gap-2">
            <p className="text-[11px] text-text-muted leading-relaxed">
              <span className="font-medium text-text-main">Purpose:</span> Use Antigravity IDE and GitHub Copilot with any provider/model from RouterDone
            </p>
            <p className="text-[11px] text-text-muted leading-relaxed">
              <span className="font-medium text-text-main">How it works:</span> Antigravity/Copilot request goes through DNS redirect, localhost MITM proxy, RouterDone, then back to the IDE.
            </p>
          </div>
          ...
          <div className="flex flex-col gap-2">
            <div className="grid gap-1 sm:grid-cols-[8rem_auto_1fr] sm:items-center sm:gap-2">
              <span className="text-xs font-semibold text-text-main sm:text-right sm:text-sm">Gateway Base URL</span>
            ...
            {!isRunning && (
              <div className="grid gap-1 sm:grid-cols-[8rem_auto_1fr] sm:items-center sm:gap-2">
                ...
                <input
                  ...
                  placeholder={cloudEnabled ? "Enter or pick API key" : "sk_routerdone (default)"}
          ...
          {serverIsWindows && !isAdmin && (
            <div className="flex items-center gap-2 px-2 py-1.5 rounded text-xs bg-red-500/10 text-red-600 border border-red-500/20">
              ...
              <span>Administrator required - restart RouterDone as Administrator to use MITM</span>
            </div>
          )}
        </div>
      </Card>
